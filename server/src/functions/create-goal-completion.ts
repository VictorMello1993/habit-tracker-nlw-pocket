...
export async function createGoalCompletions({ goalId }: CreateGoalCompletionRequest) {
	...
	const result = await db
		...
		.select({
			...
			completionCounts: sql`COALESCE(${goalCompletionsCounts.completionCount}, 0)`.mapWith(Number),
		})
		.from(goals)
